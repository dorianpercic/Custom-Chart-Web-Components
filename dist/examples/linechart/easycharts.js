...
class BarChart extends HTMLElement {
     ...
     */
    drawBarChart(width, height, dictionaries, showTicks = 'true', chartTitle = 'chartTitle') {
        const margin = {
            top: height * 0.2,
            bottom: height * 0.2,
            left: width * 0.2,
        ...
        const barChartSvg = create$1('svg')
            .attr('viewBox', `0 0 ${width} ${height}`)
            .attr('version', '1.1');
        const xScale = band().range([0, chartWidth]).padding(0.1);
        xScale.domain(Object.keys(dataDict[dataSeriesKey]));
        const yScale = linear()
            .domain([0, max(dataArray) || 0])
            .range([chartHeight, 0]);
        const xAxis = showTicks === 'true'
            ? axisBottom(xScale)
        ...
        barChartSvg
            ...
            .data(dataArray)
            .join('rect')
            .attr('x', (_, i) => margin.left + xScale(Object.keys(dataDict[dataSeriesKey])[i]))
...
class LineChart extends HTMLElement {
     ...
     * @param chartTitle - Title of the chart.
     */
    drawLineChart(width, height, dictionaries, showTicks = 'true', chartTitle = 'chartTitle') {
        const margin = {
            top: height * 0.2,
            bottom: height * 0.2,
            left: width * 0.2,
        ...
        const colors = dictionaries[2];
        const xScale = point()
            .domain(Object.keys(data).reduce((labels, seriesKey) => {
        ...
        const lineChartSvg = create$1('svg')
            .attr('viewBox', `0 0 ${width} ${height}`)
            .attr('version', '1.1');
        // Draw lines
 ...
 * second of the x and y axis titles and the third of the colors.
 */
function getTableDict(classObject) {
        ...
        if (trElement) {
            ...
            if (thElements) {
                thElements.forEach((value, index1) => {
                    ...
                    if (id || classAttr) {
                        const color = getColorFromCSS(id, classAttr);
                        if (color) {
                    ...
                    tableRows.forEach((row) => {
                        ...
                        if (cells && cells.length < 2) {
                            throw new Error('Invalid table structure: Each row must have at least two columns');
                        }
 ...
 * second of the x and y axis titles and the third of the colors.
 */
function getDataSeriesDict(classObject) {
    ...
    dataSeriesElement.forEach(function (dataSeries) {
        ...
        let dataSeriesName = dataSeries.getAttribute('name');
        if (!dataSeriesName) {
 ...
 * @param chartTitle - Title of the chart.
 */
function handleDataSeriesMode(classObject, chartWidth, chartHeight, showTicks, chartTitle) {
    try {
        const dictionaries = getDataSeriesDict(classObject);
        if (classObject instanceof BarChart) {
            classObject.drawBarChart(chartWidth, chartHeight, dictionaries, showTicks, chartTitle);
        }
        else if (classObject instanceof LineChart) {
            classObject.drawLineChart(chartWidth, chartHeight, dictionaries, showTicks, chartTitle);
        }
    }
    catch (error) {
 ...
 * @param chartTitle - Title of the chart.
 */
function handleTableMode(classObject, chartWidth, chartHeight, showTicks, chartTitle) {
    try {
 ...
 */
function setChartWidth(classObject) {
    let width = 500;
 ...
 */
function setChartHeight(classObject) {
    let height = 300;
    let containsLettersRegex = /[a-zA-Z]/g;
 ...
 */
function getAxisTitles(classObject) {
    // Set x and y axis
 ...
 */
function getSizeFromCSS(classObject) {
    ...
    const getSize = (rule) => {
        ...
        if (styleDeclaration.getPropertyValue('--chart-width')) {
            chartWidth = parseInt(styleDeclaration.getPropertyValue('--chart-width'), 10);
        }
        if (styleDeclaration.getPropertyValue('--chart-height')) {
            chartHeight = parseInt(styleDeclaration.getPropertyValue('--chart-height'), 10);
        }
    ...
    if (document.styleSheets.length !== 0) {
        try {
            ...
            const styleSheet = Array.from(document.styleSheets).find((sheet) => sheet.ownerNode instanceof HTMLStyleElement);
            if (styleSheet) {
 ...
 */
function getColorFromCSS(id, classAttr) {
    // Inner function getting the color of the line or bars
    const getColorInner = (rule) => {
        let color = 'blue';
        const styleDeclaration = rule.style;
 ...
 */
function getTicksFlagFromCSS(classObject) {
